...
# AI triage helpers (per-row confidence gauge + admin decisions). Importing
# them in a try/except so the dashboard still works if the AI sidecar can't
# load (e.g. scikit-learn missing) -- it just shows neutral confidences.
try:
    from ai_detection.triage import (
    ...
    def compute_ai_confidence(_row):     # type: ignore[no-redef]
        return 50

    def confidence_tier(c):              # type: ignore[no-redef]
        return "med"

    def load_decisions():                # type: ignore[no-redef]
        return {}

    def record_decision(*_a, **_kw):     # type: ignore[no-redef]
        raise RuntimeError(f"AI triage unavailable: {_e}")

    def get_decision(_ip):               # type: ignore[no-redef]
        return None

    def best_match_for_row(_row):        # type: ignore[no-redef]
        return None

    def decisions_path():                # type: ignore[no-redef]
        return ""
...
def calculate_risk(threat_type, severity, score):
    ...
    AI rows ("AI: ..." threat_type) are special-cased: the AI engine
    already computes a blended 0-100 threat score and stores it in `score`,
    so we trust it directly instead of re-deriving from heuristics.
    ...
    # ---- AI rows: the score IS the blended threat score (0..100).
    if t.startswith("ai:"):
...
def load_alerts(path=None):
    ...
    if not os.path.isfile(p) or os.path.getsize(p) == 0:
        return pd.DataFrame(columns=COLUMNS + ["risk", "ai_confidence", "decision"])
    try:
    ...
    # ---- AI per-row triage --------------------------------------------------
    # ai_confidence: 0..100 score the AI engine assigns to this specific row.
    # decision:      "" | "allow" | "block" | "review" -- admin's choice for
    #                this IP (persisted in ai_decisions.csv).
    decisions = load_decisions() if len(df) else {}
    df["ai_confidence"] = df.apply(
        lambda r: int(compute_ai_confidence(r.to_dict())),
        axis=1,
    ).astype(int)
    df["decision"] = df["ip"].map(
        lambda ip: (decisions.get(ip) or {}).get("decision", "")
    ).fillna("").astype(str)
    return df
...
def api_data():
    ...
    total = len(df)
    if total:
        ai_mask = df["threat_type"].astype(str).str.lower().str.startswith("ai:")
        ai_total = int(ai_mask.sum())
        ai_max_threat = int(df.loc[ai_mask, "score"].max()) if ai_total else 0
    else:
        ai_total = 0
        ai_max_threat = 0
    ...
    stats = {
        ...
        # AI sidecar visibility
        "ai_alerts":     ai_total,
        "ai_max_threat": ai_max_threat,
    }
    ...
    for rec in df.head(200).to_dict(orient="records"):
        ...
        rec["ai_confidence"] = int(rec.get("ai_confidence", 50))
        rec["decision"]      = str(rec.get("decision", ""))
        alerts.append(rec)
...
ATTACK_INFO = {
    ...
    "ai-detection": {
        "title": "AI Behavioural Detection",
        "icon": "&#129302;",
        "color": "#a371f7",
        "tagline": "ML-driven \"behaviour DNA\" matching - catches variants the signature rules miss.",
        "summary": (
            "Every row tagged <code>AI: ...</code> comes from the AI sidecar in "
            "<code>ai_detection/</code>. While the legacy detectors fire on hard "
            "thresholds (15 ports in 10s, 100 SYNs in 10s, etc.), the AI engine "
            "extracts a 26-feature \"behaviour DNA\" vector from each source IP's "
            "flow, compares it against 8 known attack fingerprints, runs an "
            "IsolationForest anomaly check, and blends those signals into a single "
            "0-100 threat score. This means the AI catches <b>variants</b> - "
            "scans that stay just under the threshold, slow-and-low brute force, "
            "beacon-like C2 traffic - that the rules would miss."
        ),
        "sections": [
            ("What the AI alert means", [
                "<code>AI: port_scan / syn_scan / brute_force / ddos</code> - the flow's behaviour-DNA matched a known attack profile with high similarity.",
                "<code>AI: beaconing / lateral_movement / dns_tunneling / arp_spoof</code> - profiles the legacy rule engine doesn't cover at all.",
                "<code>AI: anomalous behaviour</code> - no profile matched, but the IsolationForest model flagged the flow as far from the normal-traffic baseline.",
                "The <b>Score</b> column (0-100) is the AI's blended threat score, not the raw packet count. 70+ is an alert; 85+ is critical.",
            ]),
            ("How it works", [
                "<b>Feature extractor:</b> turns each source IP's last 10s of traffic into 26 numbers - packet rate, SYN/ACK ratios, unique ports, destination entropy, inter-arrival variance, etc.",
                "<b>Similarity engine:</b> cosine + weighted Euclidean + range-hit-rate against every fingerprint in <code>ai_detection/attack_profiles.py</code>.",
                "<b>Anomaly detector:</b> scikit-learn IsolationForest trained on a normal-traffic baseline. <code>decision_function</code> is rescaled to 0..1.",
                "<b>Threat scoring:</b> 45% similarity + 25% anomaly + 15% IoC hit + 10% legacy-detector hit + 5% traffic intensity, capped to 0..100.",
                "<b>Worker thread:</b> all of this runs in a background thread - packet capture is never blocked.",
            ]),
            ("Configuring thresholds", [
                "<code>SIMILARITY_THRESHOLD</code> (default 0.80) - cosine match required before \"matched a profile\" fires.",
                "<code>ANOMALY_THRESHOLD</code> (default 0.65) - IsolationForest score required for the standalone anomaly alert.",
                "<code>THREAT_ALERT_THRESHOLD</code> (default 70) - the blended 0-100 score above which an AI alert is written to the log.",
                "All three are env vars; defaults live in <code>ai_detection/config.py</code>. Set <code>AI_ENABLED=false</code> to disable the engine entirely (the legacy IDS keeps running).",
            ]),
            ("Replacing the baseline model", [
                "On first run the engine fits a synthetic normal-traffic baseline so it's usable immediately.",
                "For accurate alerts in your environment, capture a few hours of quiet traffic and run <code>python train_ai_baseline.py quiet.pcap</code>.",
                "The retrained model is persisted to <code>ai_detection/models/isoforest.joblib</code> and auto-loaded next start.",
                "The engine can also auto-retrain on observed normal traffic every <code>MODEL_RETRAIN_INTERVAL</code> seconds (default 24h).",
            ]),
            ("Why have both AI and rules?", [
                "Rules are <b>fast and explainable</b> - perfect for known attacks. The legacy engine still fires unchanged.",
                "AI is <b>fuzzy and adaptive</b> - perfect for variants and unknowns. It's a sidecar, not a replacement.",
                "Two layers means a stealth-tuned port scan that dodges the 15-port-in-10s rule can still match the <code>port_scan</code> behaviour-DNA profile.",
                "If both fire on the same IP, you'll see one legacy row plus one <code>AI:</code> row in the log - same source, two different reasons.",
            ]),
        ],
    },
    "malicious-ip": {
...
def api_ai_decision():
    """Record an admin decision (allow / block / review) for one IP."""
    payload = request.get_json(silent=True) or {}
    ip       = str(payload.get("ip", "")).strip()
    decision = str(payload.get("decision", "")).strip().lower()
    notes    = str(payload.get("notes", "")).strip()
    threat_type = str(payload.get("threat_type", ""))
    score    = int(payload.get("score") or 0)
    if not _IP_RE.match(ip):
        return jsonify(ok=False, error="invalid ip"), 400
    if decision not in ("allow", "block", "review"):
        return jsonify(ok=False, error="decision must be allow/block/review"), 400
    try:
        row = record_decision(ip, decision, threat_type, score, notes)
        return jsonify(ok=True, decision=row)
    except Exception as e:
        return jsonify(ok=False, error=str(e)), 500


@app.route("/ai/review/<ip>")
def ai_review(ip):
    """AI review page for a single source IP -- per-row breakdown + Allow/Block."""
    if not _IP_RE.match(ip):
        return ("<p style='color:#e6edf3;background:#0d1117;padding:24px;"
                "font-family:sans-serif;margin:0;min-height:100vh;'>"
                "Invalid IP. <a href='/' style='color:#58a6ff'>Back</a></p>"), 400

    df = load_alerts()
    if len(df):
        df_ip = df[df["ip"] == ip].sort_values("timestamp", ascending=False)
    else:
        df_ip = df

    rows = []
    for rec in df_ip.to_dict(orient="records"):
        rec["score"]         = int(rec.get("score", 0))
        rec["risk"]          = int(rec.get("risk", 0))
        rec["ai_confidence"] = int(rec.get("ai_confidence", 0))
        rec["decision"]      = str(rec.get("decision", ""))
        rows.append(rec)

    # Aggregate "what does the AI think about this IP overall?"
    if rows:
        max_conf = max(r["ai_confidence"] for r in rows)
        avg_conf = int(round(sum(r["ai_confidence"] for r in rows) / len(rows)))
        sample = max(rows, key=lambda r: r["ai_confidence"])
        match = best_match_for_row(sample)
    else:
        max_conf = avg_conf = 0
        sample = None
        match = None

    types = sorted({r["threat_type"] for r in rows}) if rows else []
    severities = sorted({r["severity"] for r in rows}) if rows else []

    decision = get_decision(ip) or {}
    ...
    payload = {
        ...
        "ai_match":   None if match is None else {
    ...
    html = AI_REVIEW_PAGE.replace("__PAYLOAD__", json.dumps(payload))
...
<style>
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  .ai-badge { display:inline-block; padding:1px 7px; border-radius:10px;
              font-size:10px; font-weight:600; letter-spacing:.5px;
              background:rgba(163,113,247,.15); color:#a371f7;
              border:1px solid rgba(163,113,247,.4);
              margin-right:6px; vertical-align:middle; }
  .card.ai .value { color:#a371f7; }

  /* AI half-circle gauge -- one per threat row. Click opens the review page.
     IMPORTANT: arc paths get `stroke` only (no fill) so they render as a
     thin arc, not a solid half-pie. The number is fill-only on the text. */
  .ai-gauge { width:52px; height:34px; cursor:pointer; display:inline-block;
              vertical-align:middle; transition:transform .12s; overflow:visible; }
  .ai-gauge:hover { transform:scale(1.08); }
  .ai-gauge path  { fill:none !important; stroke-linecap:round; }
  .ai-gauge .gtrack { stroke:#0a0e14; stroke-width:4; }
  .ai-gauge .gfill  { stroke-width:4; transition:stroke-dasharray .4s; }
  .ai-gauge .gnum   { font-family:Consolas,monospace; font-weight:700;
                      font-size:12px; text-anchor:middle;
                      dominant-baseline:central;
                      paint-order:stroke; stroke:#0d1117; stroke-width:3;
                      stroke-linejoin:round; }
  .ai-gauge.t-crit .gfill { stroke:#f85149; }
  .ai-gauge.t-high .gfill { stroke:#f0883e; }
  .ai-gauge.t-med  .gfill { stroke:#d29922; }
  .ai-gauge.t-low  .gfill { stroke:#3fb950; }
  .ai-gauge.t-crit .gnum  { fill:#f85149; }
  .ai-gauge.t-high .gnum  { fill:#f0883e; }
  .ai-gauge.t-med  .gnum  { fill:#d29922; }
  .ai-gauge.t-low  .gnum  { fill:#3fb950; }

  /* Decision pill -- shows under the gauge when the admin has chosen. */
  .dec-pill { font-size:9px; font-weight:700; letter-spacing:.4px;
              padding:1px 6px; border-radius:8px; margin-top:2px;
              display:inline-block; text-transform:uppercase; }
  .dec-pill.allow  { background:rgba(63,185,80,.18);  color:var(--low);
                     border:1px solid rgba(63,185,80,.4); }
  .dec-pill.block  { background:rgba(248,81,73,.18);  color:var(--high);
                     border:1px solid rgba(248,81,73,.4); }
  .dec-pill.review { background:rgba(88,166,255,.18); color:var(--accent);
                     border:1px solid rgba(88,166,255,.4); }
  .ai-cell { text-align:center; min-width:60px; }
    ...
    <div class="panel">
      ...
      <table>
        <thead><tr>
          ...
          <th class="ai-cell" title="AI confidence this is a real threat (click any to review)">AI</th>
        </tr></thead>
        <tbody id="rows"></tbody>
      </table>
    </div>
...
<script>
  ...
  function infoKey(threat_type) {
    ...
    // AI rows: route to the right *attack* info page when the AI's
    // verdict maps to an existing one, otherwise to the generic AI page.
    if (t.startsWith('ai:')) {
      if (t.includes('port_scan') || t.includes('port scan')) return 'portscan';
      if (t.includes('syn_scan'))                              return 'portscan';
      if (t.includes('brute_force') || t.includes('brute'))    return 'bruteforce';
      if (t.includes('ddos') || t.includes('flood') || t.includes('dos'))
                                                               return 'dos';
      return 'ai-detection';
    }
  ...
  // True if this is an AI-generated row (used for the "AI" badge in the table).
  function isAiRow(threat_type) {
    return (threat_type || '').toLowerCase().startsWith('ai:');
  }

  // Tier for the AI confidence gauge (same bands as the risk gauge).
  function confTier(c) {
    if (c >= 80) return 'crit';
    if (c >= 60) return 'high';
    if (c >= 40) return 'med';
  ...
  // Half-circle gauge SVG. The arc path length is 100 so stroke-dasharray
  // doubles as a percentage. Clicking it opens /ai/review/<ip> in a new tab.
  function aiGaugeHtml(conf, ip, decision) {
    ...
    const pill  = (dec === 'allow' || dec === 'block' || dec === 'review')
                  ? `<div class="dec-pill ${dec}">${dec}</div>` : '';
    // fill="none" set as a presentation attribute on each path so it cannot
    // be overridden by stale / cached CSS rules. The arc is a 36-unit-wide
    // semicircle centred at (23, 26) with radius 18.
    return `<a href="/ai/review/${ipEsc}" target="_blank"
                title="AI confidence ${c}/100 - click to review and decide"
                onclick="event.stopPropagation();" style="text-decoration:none;">
              <svg class="ai-gauge t-${tier}" viewBox="0 0 46 32" width="52" height="36">
                <path class="gtrack"
                      d="M5,26 A18,18 0 0,1 41,26"
                      fill="none" stroke="#0a0e14" stroke-width="4"
  ...
  function render() {
    ...
    if (!data.length) {
      rows.innerHTML = '<tr><td colspan="8" class="empty">No attacks detected yet.</td></tr>';
      return;
    }
    rows.innerHTML = data.map((a, i) => {
      const key = infoKey(a.threat_type);
      const ai = isAiRow(a.threat_type);
      const badge = ai
        ? `<span class="ai-badge" title="Detected by the AI behavioural sidecar">AI</span>`
        : '';
      // Hide the "AI: " prefix in the cell -- the badge already says it.
      const label = ai
        ? a.threat_type.replace(/^AI:\\s*/i, '').replace(/_/g, ' ')
        : a.threat_type;
      const typeCell = key
        ? `${badge}<a class="attack-link" data-key="${key}" title="Learn about this attack">${label}</a>`
        : `${badge}${label}`;
      return `
      <tr class="clickable" data-i="${i}" title="Click row to simulate this attack on the topology">
        ...
        <td class="ai-cell">${aiGaugeHtml(a.ai_confidence, a.ip, a.decision)}</td>
      </tr>`;
    }).join('');
  ...
  async function refresh() {
    try {
      ...
      const aiSubtitle = (s.ai_alerts || 0) > 0
        ? `max threat ${s.ai_max_threat || 0}/100 → What is this?`
        : 'AI sidecar idle → What is this?';
      document.getElementById('cards').innerHTML = [
        ['Total alerts',     s.total,           null,             null, null],
        ['Unique attackers', s.unique_ips,      null,             null, null],
        ['Inbound',          s.inbound,         null,             null, null],
        ['Outbound',         s.outbound,        null,             null, null],
        ['High severity',    s.high,            null,             null, null],
        ['AI detections',    s.ai_alerts || 0,  '#a371f7',        '/info/ai-detection', aiSubtitle],
        ['Max risk',         s.max_risk || 0,   maxRiskColor,     null, null],
        ['Avg risk',         s.avg_risk || 0,   null,             null, null],
        ['Last alert',       fmtTime(s.last),   null,             null, null]
      ].map(([l, v, c, href, sub]) => {
        const inner = `<div class="label">${l}</div>
         <div class="value"${c?` style="color:${c}"`:''}>${v}</div>` +
          (sub ? `<div class="label" style="margin-top:4px;text-transform:none;letter-spacing:0;">${sub}</div>` : '');
        if (href) {
          return `<a class="card" href="${href}" target="_blank"
                     style="text-decoration:none;display:block;cursor:pointer;">${inner}</a>`;
        }
...
<style>
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  .summary { background:var(--card); border:1px solid var(--border);
             border-left:6px solid var(--ai); border-radius:10px;
             padding:22px 26px; margin-bottom:22px;
             display:grid; grid-template-columns:auto 1fr; gap:24px;
             align-items:center; }
  .gauge-big { width:180px; height:110px; }
  .gauge-big .gtrack { stroke:#0a0e14; stroke-width:10; fill:none;
                       stroke-linecap:round; }
  .gauge-big .gfill  { stroke-width:10; fill:none; stroke-linecap:round; }
  .gauge-big.t-crit .gfill { stroke:var(--high); }
  .gauge-big.t-high .gfill { stroke:#f0883e; }
  .gauge-big.t-med  .gfill { stroke:var(--med); }
  .gauge-big.t-low  .gfill { stroke:var(--low); }
  .gauge-big text   { font-family:Consolas,monospace; font-weight:700; }
  .gauge-big .gnum  { font-size:36px; text-anchor:middle; dominant-baseline:central; }
  .gauge-big.t-crit .gnum { fill:var(--high); }
  .gauge-big.t-high .gnum { fill:#f0883e; }
  .gauge-big.t-med  .gnum { fill:var(--med); }
  .gauge-big.t-low  .gnum { fill:var(--low); }
  .glabel { fill:var(--muted); font-size:11px; text-anchor:middle;
            letter-spacing:.5px; font-family:'Segoe UI',sans-serif; }
  .meta dl { display:grid; grid-template-columns:auto 1fr; gap:6px 14px;
             font-size:13px; }
  .meta dt { color:var(--muted); }
  .meta dd { color:var(--text); font-family:Consolas,monospace; }
  .panel { background:var(--card); border:1px solid var(--border);
  ...
  .medium { background:rgba(210,153,34,.15); color:var(--med); }
  .low    { background:rgba(63,185,80,.15);  color:var(--low); }
  .matched { display:flex; flex-wrap:wrap; gap:8px; }
  .matched span { background:rgba(163,113,247,.12); color:var(--ai);
                  border:1px solid rgba(163,113,247,.35);
                  border-radius:6px; padding:4px 10px; font-size:12px; }
  .actions { display:flex; gap:10px; flex-wrap:wrap; align-items:center; }
  .actions button { background:var(--bg); color:var(--text);
    border:1px solid var(--border); padding:10px 16px; border-radius:6px;
    cursor:pointer; font-size:14px; font-weight:600; transition:all .15s; }
  .actions .allow:hover  { background:var(--low); color:#0a0e14;
                           border-color:var(--low); }
  .actions .block:hover  { background:var(--high); color:#fff;
                           border-color:var(--high); }
  .actions .review:hover { background:var(--accent); color:#fff;
                           border-color:var(--accent); }
  .actions textarea { background:var(--bg); color:var(--text);
    border:1px solid var(--border); border-radius:6px; padding:8px 12px;
    font-family:inherit; font-size:13px; min-width:260px; flex:1;
    resize:vertical; min-height:42px; }
  .current-decision { display:inline-block; padding:6px 12px;
    border-radius:8px; font-size:12px; font-weight:600;
    text-transform:uppercase; letter-spacing:.4px; }
  .current-decision.allow  { background:rgba(63,185,80,.18);  color:var(--low);
                              border:1px solid rgba(63,185,80,.4); }
  .current-decision.block  { background:rgba(248,81,73,.18);  color:var(--high);
                              border:1px solid rgba(248,81,73,.4); }
  .current-decision.review { background:rgba(88,166,255,.18); color:var(--accent);
                              border:1px solid rgba(88,166,255,.4); }
  .current-decision.none   { background:#0a0e14; color:var(--muted);
                              border:1px solid var(--border); }
  .feedback { margin-top:10px; font-size:12.5px; color:var(--muted); }
  .feedback.ok  { color:var(--low); }
  .feedback.err { color:var(--high); }
  .empty { color:var(--muted); text-align:center; padding:20px; }
</style>
</head>
<body>
  <div class="crumb">
    <a href="/">&larr; Back to dashboard</a> &middot;
    <a href="/info/ai-detection">What is the AI doing?</a>
  </div>

  <h1 id="title">AI Review</h1>
  <div class="sub" id="sub"></div>

  <div class="summary">
    <svg class="gauge-big" id="gauge" viewBox="0 0 200 110">
      <path class="gtrack"
            d="M20,100 A80,80 0 0,1 180,100" pathLength="100"/>
      <path class="gfill" id="gauge-fill"
            d="M20,100 A80,80 0 0,1 180,100" pathLength="100"
            stroke-dasharray="0 100"/>
      <text class="gnum" id="gauge-num" x="100" y="80">0</text>
      <text class="glabel" x="100" y="105">AI confidence / 100</text>
    </svg>
    <div class="meta">
      <dl id="meta-dl"></dl>
    </div>
  ...
  <div class="panel">
    ...
    <div id="ai-verdict" style="font-size:14px; line-height:1.6;"></div>
    <div class="matched" id="matched" style="margin-top:12px;"></div>
  </div>

  <div class="panel">
    <h2>Admin Decision</h2>
    <p style="font-size:13px; color:var(--muted); margin-bottom:14px;">
      Current decision for this IP:
      <span id="current-decision" class="current-decision none">none</span>
      <span id="decision-time" style="margin-left:8px; font-family:Consolas,monospace;
            font-size:11.5px; color:var(--muted);"></span>
    </p>
    <div class="actions">
      <textarea id="notes" placeholder="Optional notes (why this decision?)"></textarea>
      <button class="allow"  onclick="submitDecision('allow')">
        &#9989; Allow (let it through)
      </button>
      <button class="block"  onclick="submitDecision('block')">
        &#9940; Block (confirmed threat)
      </button>
      <button class="review" onclick="submitDecision('review')">
        &#128270; Needs review
      </button>
    </div>
  ...
  <div class="panel">
    <h2>All detections from this IP</h2>
    <table>
      <thead><tr>
        <th>Time</th><th>Type</th><th>Direction</th>
        <th>Severity</th><th>Score</th><th>Risk</th><th>AI</th>
      </tr></thead>
      <tbody id="rows"></tbody>
    </table>
  </div>
...
<script>
  ...
  document.getElementById('title').textContent = `AI Review: ${data.ip}`;
  document.getElementById('sub').textContent =
    `${data.rows.length} detections logged for this source IP. ` +
    `The AI assigns a 0-100 confidence to each row -- higher means more likely a real threat.`;

  // ---- Big gauge -------------------------------------------------------
  const max = data.max_conf || 0;
  const t   = tier(max);
  const g   = document.getElementById('gauge');
  g.classList.add('t-' + t);
  document.getElementById('gauge-fill')
    .setAttribute('stroke-dasharray', max + ' 100');
  document.getElementById('gauge-num').textContent = max;

  // ---- Meta block ------------------------------------------------------
  const m = document.getElementById('meta-dl');
  const ai = data.ai_match;
  const rows = data.rows || [];
  const lastTs = rows.length ? rows[0].timestamp : '';
  m.innerHTML = `
    <dt>Source IP</dt><dd>${esc(data.ip)}</dd>
    <dt>Detections</dt><dd>${rows.length}</dd>
    <dt>Max AI confidence</dt><dd>${max}/100 (${t})</dd>
    <dt>Avg AI confidence</dt><dd>${data.avg_conf}/100</dd>
    <dt>Threat types</dt><dd>${data.types.map(esc).join(', ') || '\\u2014'}</dd>
    <dt>Severities</dt><dd>${data.severities.map(esc).join(', ') || '\\u2014'}</dd>
    <dt>Last seen</dt><dd>${esc(fmtTime(lastTs))}</dd>
  `;

  // ---- AI verdict ------------------------------------------------------
  const verdict = document.getElementById('ai-verdict');
  if (ai) {
    verdict.innerHTML = `Best behavioural match: <b>${esc(ai.attack_type)}</b>
       (similarity ${(ai.similarity * 100).toFixed(1)} / 100,
       confidence band <b>${esc(ai.confidence)}</b>).
       <br>This means the source IP's behaviour profile most closely resembles
       <code>${esc(ai.attack_type)}</code> traffic patterns from the AI's
       fingerprint library.`;
  } else {
    verdict.textContent = 'No AI match available for this IP yet.';
  }
  const matched = document.getElementById('matched');
  const feats = (ai && ai.matched_features) || [];
  matched.innerHTML = feats.length
    ? feats.map(f => `<span>${esc(f)}</span>`).join('')
    : '<span style="background:transparent;border:none;color:var(--muted);">' +
      'No specific feature flags matched -- decision should weigh other signals.</span>';

  // ---- Current decision ------------------------------------------------
  function paintDecision(dec, ts) {
    const el = document.getElementById('current-decision');
    el.className = 'current-decision ' + (dec || 'none');
    el.textContent = dec || 'none';
    document.getElementById('decision-time').textContent =
      ts ? `(set ${fmtTime(ts)})` : '';
  }
  paintDecision(data.decision.decision || '', data.decision.timestamp || '');

  // ---- Detections table ------------------------------------------------
  const tbody = document.getElementById('rows');
  if (!rows.length) {
    tbody.innerHTML = '<tr><td colspan="7" class="empty">No detections for this IP.</td></tr>';
  } else {
    tbody.innerHTML = rows.map(r => {
      const ct = tier(r.ai_confidence || 0);
      return `<tr>
        ...
        <td>${r.score}</td>
        <td>${r.risk || 0}</td>
        <td><span style="color:${ct==='crit'?'var(--high)':ct==='high'?'#f0883e':ct==='med'?'var(--med)':'var(--low)'};
                  font-family:Consolas,monospace; font-weight:700;">${r.ai_confidence}</span></td>
      </tr>`;
    }).join('');
  }
  ...
  async function submitDecision(dec) {
    ...
    try {
      ...
      paintDecision(j.decision.decision, j.decision.timestamp);
      fb.className = 'feedback ok';
      fb.textContent = `Saved: ${dec} for ${data.ip}.`;
    } catch (e) {
      fb.className = 'feedback err';
      fb.textContent = 'Failed to save: ' + e.message;
    }
  }
